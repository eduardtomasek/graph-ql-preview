...
import { MAIN_PG_CONNECTION, PG_OID_DATE } from 'src/libs/core/constants';
...
const initOptions: IInitOptions = {
    ...
    noWarnings: true,
    connect(client) {
        const appName = process.env.PG_APP_NAME ?? 'vyra-api';
        /*
...
const logger = new Logger('MainDbProvider');

export const mainDbProvider = {
    provide: MAIN_PG_CONNECTION,
    useFactory: (configService: ConfigService): IDatabase<object> => {
        const dbUrl = configService.getOrThrow<string>('MAIN_DB_URL');
        const cs = new ConnectionString(dbUrl);
        if (!cs?.hosts?.[0]?.name) {
            throw new Error('MAIN_DB_URL is invalid');
        }
        ...
        const ssl = isProd
            ? {
                  ca: fs.readFileSync(path.join(__dirname, '../../../assets/us-east-1-bundle.pem')).toString(),
        ...
        const connConfig = {
            ...
            ssl: isProd ? ssl : undefined, // SSL zapni jen v produkci
            /*
            ...
            */
            max: Number(configService.get('PG_POOL_MAX') ?? 10), // počet připojení v poolu
            idleTimeoutMillis: Number(configService.get('PG_POOL_IDLE_MS') ?? 300_000), // 5 min – jak dlouho čeká neaktivní klient v poolu, než ho zahodí
            connectionTimeoutMillis: Number(configService.get('PG_CONN_TIMEOUT_MS') ?? 2000),
            keepAlive: true, // TCP keepalive nech ON, ať se méně reconnectuje a lépe zjišťují mrtvá spojení
            keepAliveInitialDelayMillis: Number(configService.get('PG_KA_INIT_DELAY_MS') ?? 30_000), // posílej keepalive brzy (nečekej hodiny)
        };
        ...
        void db
            .connect()
            .then((c) => {
                logger.log(`Connected to database: ${cs.hosts![0].name}:${cs.hosts![0].port ?? 5432}/${cs.path?.[0]}`);
                void c.done();
            })
            .catch((err: Error) => {
